...
class SS2Dv2:                                       #go for this
    def __initv2__(
        ...
        **kwargs,    
        ...
        # in proj =======================================
        d_proj = d_inner if self.disable_z else (d_inner * 2)   #d_inner * 2
        ...
        if initialize in ["v0"]:    #go for this
            # dt proj ============================
            self.dt_projs = [
                self.dt_init(dt_rank, d_inner, dt_scale, dt_init, dt_min, dt_max, dt_init_floor)
                for _ in range(k_group)
        ...
        **kwargs,
        ...
        if cascade2d:            #false
            ...
            if no_einsum:
                ...
                if x_proj_bias is not None:          #false
        ...
        return (y.to(x.dtype) if to_dtype else y), As, x_proj_weight

    def forwardv2(self, x: torch.Tensor, A_weight=None, B_weight=None, **kwargs):
        x = self.in_proj(x)
        ...
        x = self.act(x)
        if self.block_th > 0:
            y, As, Bs = self.forward_core(x, A_weight, B_weight)
        else:
            y, As, Bs = self.forward_core(x)
        y = self.out_act(y)
        if not self.disable_z:
            y = y * z
        out = self.dropout(self.out_proj(y))
    ...
    def __init__(
        ...
        **kwargs,
        ...
        kwargs.update(
            ...
            initialize=initialize, forward_type=forward_type, channel_first=channel_first, block_th=block_th,
        )
        if forward_type in ["v0", "v0seq"]:
            ...
            self.__initv2__(**kwargs)   #go for this
...
class VSSBlock(nn.Module):
    ...
    def _forward(self, input: torch.Tensor, A_weight_old=None, B_weight_old=None):
        x = input
        if self.ssm_branch:
            if self.post_norm:   #false
                x = x + self.drop_path(self.norm(self.op(x)))
            else:
                # x = x + self.drop_path(self.op(self.norm(x)))
                x0, A_weight_new, B_weight_new = self.op(self.norm(x), A_weight_old, B_weight_old)
                x = x + self.drop_path(x0)
        if self.mlp_branch:
            if self.post_norm:
                x = x + self.drop_path(self.norm2(self.mlp(x))) # FFN
            else:
                x = x + self.drop_path(self.mlp(self.norm2(x))) # FFN
        return x, A_weight_new, B_weight_new
...
class VSSM(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        super().__init__()
        self.channel_first = (norm_layer.lower() in ["bn", "ln2d"])      #false
        ...
        for i_layer in range(self.num_layers):
            ...
            self.layers_hsi.append(self._make_layer(
        ...
        for i_layer in range(self.num_layers):
            ...
            self.layers_lidar.append(self._make_layer(
                dim = self.dims[i_layer],
                drop_path = dpr[sum(depths[:i_layer]):sum(depths[:i_layer + 1])],
                use_checkpoint=use_checkpoint,
                norm_layer=norm_layer,
    ...
    def forward(self, x: torch.Tensor, y: torch.Tensor):
        ...
        z = x + y
        for layer_hsi, layer_lidar in zip(self.layers_hsi, self.layers_lidar):
            for i, block_hsi, block_lidar in enumerate(zip(layer_hsi, layer_lidar)):
                if i == (len(layer_hsi) - 1):
                    x = block_hsi(x)
                    y = block_lidar(y)
                elif i > 0:
                    x, A_HSI_weight, B_HSI_weight = block_hsi(x, A_HSI_weight, B_HSI_weight)
                    y, A_LIDAR_weight, B_LIDAR_weight = block_lidar(y, A_LIDAR_weight, B_LIDAR_weight)
                else:
                    x, A_HSI_weight, B_HSI_weight = block_hsi(x)
                    y, A_LIDAR_weight, B_LIDAR_weight = block_lidar(y)
                tmp_B = B_LIDAR_weight
                B_LIDAR_weight = B_HSI_weight
                B_HSI_weight = tmp_B
                tmp_A = (A_HSI_weight + A_LIDAR_weight) / 2
                A_HSI_weight = tmp_A
                A_LIDAR_weight = tmp_A
            # x = layer(x)
        x = self.classifier_hsi(x)
        y = self.classifier_hsi(y)
        return x
