...
class SS2Dv2:
    def __initv2__(
        ...
        **kwargs,    
        ...
        self.x_proj_weight_hsi = nn.Parameter(torch.stack([t.weight for t in self.x_proj_hsi], dim=0)) # (K, N, inner)
        del self.x_proj_hsi
        self.x_proj_weight_lidar = nn.Parameter(torch.stack([t.weight for t in self.x_proj_lidar], dim=0)) # (K, N, inner)
        del self.x_proj_lidar
        ...
        if initialize in ["v0"]:
            ...
            self.dt_projs_weight_hsi = nn.Parameter(torch.stack([t.weight for t in self.dt_projs_hsi], dim=0)) # (K, inner, rank)
            self.dt_projs_bias_hsi = nn.Parameter(torch.stack([t.bias for t in self.dt_projs_hsi], dim=0)) # (K, inner)
            self.dt_projs_weight_lidar = nn.Parameter(torch.stack([t.weight for t in self.dt_projs_lidar], dim=0)) # (K, inner, rank)
            self.dt_projs_bias_lidar = nn.Parameter(torch.stack([t.bias for t in self.dt_projs_lidar], dim=0)) # (K, inner)
            del self.dt_projs_hsi
            del self.dt_projs_lidar
            
            # A, D =======================================
            self.A_logs_hsi = self.A_log_init(d_state, d_inner, copies=k_group, merge=True) # (K * D, N)
            self.Ds_hsi = self.D_init(d_inner, copies=k_group, merge=True) # (K * D)
            self.A_logs_lidar = self.A_log_init(d_state, d_inner, copies=k_group, merge=True) # (K * D, N)
            self.Ds_lidar = self.D_init(d_inner, copies=k_group, merge=True) # (K * D)
        elif initialize in ["v1"]:
            # simple init dt_projs, A_logs, Ds
            self.Ds = nn.Parameter(torch.ones((k_group * d_inner)))
            self.A_logs = nn.Parameter(torch.randn((k_group * d_inner, d_state))) # A == -A_logs.exp() < 0; # 0 < exp(A * dt) < 1
    ...
    def forward_corev2(
        ...
        **kwargs,
        ...
        x_proj_weight = self.x_proj_weight_hsi
        y_proj_weight = self.x_proj_weight_lidar
        x_proj_bias = getattr(self, "x_proj_bias", None)
        y_proj_bias = getattr(self, "y_proj_bias", None)
        dt_projs_weight_hsi = self.dt_projs_weight_hsi
        dt_projs_bias_hsi = self.dt_projs_bias_hsi
        dt_projs_weight_lidar = self.dt_projs_weight_lidar
        dt_projs_bias_lidar = self.dt_projs_bias_lidar
        A_logs_hsi = self.A_logs_hsi
        A_logs_lidar = self.A_logs_lidar
        Ds_hsi = self.Ds_hsi
        Ds_lidar = self.Ds_lidar
        delta_softplus = True
        out_norm = getattr(self, "out_norm", None)
        channel_first = self.channel_first
        to_fp32 = lambda *args: (_a.to(torch.float32) for _a in args)
        ...
        B, D, H, W = x.shape
        D, N = A_logs_hsi.shape
        K, D, R = dt_projs_weight_hsi.shape
        L = H * W
        ...
        x_xs = CrossScan.apply(x)
        y_xs = CrossScan.apply(y)
        if no_einsum:
            # x_dbl = F.conv1d(xs.view(B, -1, L), x_proj_weight.view(-1, D, 1), bias=(x_proj_bias.view(-1) if x_proj_bias is not None else None), groups=K)
            dts, Bs, Cs = torch.split(x_dbl.view(B, K, -1, L), [R, N, N], dim=2)
            # dts = F.conv1d(dts.contiguous().view(B, -1, L), dt_projs_weight.view(K * D, -1, 1), groups=K)
        else:
            x_dbl = torch.einsum("b k d l, k c d -> b k c l", x_xs, x_proj_weight)
            y_dbl = torch.einsum("b k d l, k c d -> b k c l", y_xs, y_proj_weight)
            if x_proj_bias is not None:
                x_dbl = x_dbl + x_proj_bias.view(1, K, -1, 1)
            if y_proj_bias is not None:
                y_dbl = y_dbl + y_proj_bias.view(1, K, -1, 1)
            x_dts, x_Bs, x_Cs = torch.split(x_dbl, [R, N, N], dim=2)
            y_dts, y_Bs, y_Cs = torch.split(y_dbl, [R, N, N], dim=2)
            x_dts = torch.einsum("b k r l, k d r -> b k d l", x_dts, dt_projs_weight_hsi)
            y_dts = torch.einsum("b k r l, k d r -> b k d l", y_dts, dt_projs_weight_lidar)

        x_xs = x_xs.view(B, -1, L)
        y_xs = y_xs.view(B, -1, L)
        x_dts = x_dts.contiguous().view(B, -1, L)
        y_dts = y_dts.contiguous().view(B, -1, L)
        x_As = -torch.exp(A_logs_hsi.to(torch.float)) # (k * c, d_state)
        y_As = -torch.exp(A_logs_lidar.to(torch.float)) # (k * c, d_state)
        x_Bs = x_Bs.contiguous().view(B, K, N, L)
        x_Cs = x_Cs.contiguous().view(B, K, N, L)
        x_Ds = Ds_hsi.to(torch.float) # (K * c)
        y_Bs = y_Bs.contiguous().view(B, K, N, L)
        y_Cs = y_Cs.contiguous().view(B, K, N, L)
        y_Ds = Ds_lidar.to(torch.float) # (K * c)
        x_delta_bias = dt_projs_bias_hsi.view(-1).to(torch.float)
        y_delta_bias = dt_projs_bias_lidar.view(-1).to(torch.float)

        if force_fp32:
            x_xs, x_dts, x_Bs, x_Cs = to_fp32(x_xs, x_dts, x_Bs, x_Cs)
            y_xs, y_dts, y_Bs, y_Cs = to_fp32(y_xs, y_dts, y_Bs, y_Cs)

        #process A and B here
        xy_As = (x_As + y_As) / 2

        x_ys: torch.Tensor = selective_scan(
            x_xs, x_dts, xy_As, y_Bs, x_Cs, x_Ds, x_delta_bias, delta_softplus
        ).view(B, K, -1, H, W)
        y_ys: torch.Tensor = selective_scan(
            y_xs, y_dts, xy_As, x_Bs, y_Cs, y_Ds, y_delta_bias, delta_softplus
        ).view(B, K, -1, H, W)
        
        x_y: torch.Tensor = CrossMerge.apply(x_ys)
        y_y: torch.Tensor = CrossMerge.apply(y_ys)

        if getattr(self, "__DEBUG__", False):
            setattr(self, "__datahsi__", dict(
                A_logs=A_logs_hsi, Bs=x_Bs, Cs=x_Cs, Ds=x_Ds,
                us=x_xs, dts=x_dts, delta_bias=x_delta_bias,
                ys=x_ys, y=x_y,
            ))
            setattr(self, "__datalidar__", dict(
                A_logs=A_logs_lidar, Bs=y_Bs, Cs=y_Cs, Ds=y_Ds,
                us=y_xs, dts=y_dts, delta_bias=y_delta_bias,
                ys=y_ys, y=y_y,
            ))

        x_y = x_y.view(B, -1, H, W)
        y_y = y_y.view(B, -1, H, W)
        if not channel_first:
            x_y = x_y.view(B, -1, H * W).transpose(dim0=1, dim1=2).contiguous().view(B, H, W, -1) # (B, L, C)
            y_y = y_y.view(B, -1, H * W).transpose(dim0=1, dim1=2).contiguous().view(B, H, W, -1) # (B, L, C)
        x_y = out_norm(x_y)
        y_y = out_norm(y_y)

        return (x_y.to(x.dtype) if to_dtype else x_y), (y_y.to(y.dtype) if to_dtype else y_y)

    def forwardv2(self, x: torch.Tensor, y: torch.Tensor, **kwargs):
        x = self.in_proj(x)
        y = self.in_proj(y)
        if not self.disable_z:
            x, x_z = x.chunk(2, dim=(1 if self.channel_first else -1)) # (b, h, w, d)
            y, y_z = y.chunk(2, dim=(1 if self.channel_first else -1)) # (b, h, w, d)
            if not self.disable_z_act:
                x_z = self.act(x_z)
                y_z = self.act(y_z)
        if not self.channel_first:
        ...
        if self.with_dconv:
            x = self.conv2d(x) # (b, d, h, w)
            y = self.conv2d(y) # (b, d, h, w)
        x = self.act(x)
        y = self.act(y)
        x, y = self.forward_core(x, y)
        x = self.out_act(x)
        y = self.out_act(y)
        if not self.disable_z:
            x = x * x_z
            y = y * y_z
        out_x = self.dropout(self.out_proj(x))
        out_y = self.dropout(self.out_proj(y))
        return out_x, out_y
    #=============================================================================================#
    def forward_corev2_none(
        self,
        x: torch.Tensor=None, 
        # ==============================
        to_dtype=True, # True: final out to dtype
    ...
    def forwardv2_none(self, x: torch.Tensor, **kwargs):
        x = self.in_proj(x)
        if not self.disable_z:
            x, z = x.chunk(2, dim=(1 if self.channel_first else -1)) # (b, h, w, d)
            if not self.disable_z_act:
    ...
    def forward_go(self, input_x: torch.Tensor, input_y = None):
        if input_y is None:
            return self.forwardv2_none(input_x)
        else:
            return self.forwardv2(input_x, input_y)
...
class SS2D_add(nn.Module, mamba_init, SS2Dv0, SS2Dv2_add, SS2Dv3):
    def __init__(
        self,
        # basic dims ===========
        d_model=96,
...
class VSSBlock(nn.Module):
    ...
    def _forward(self, input_x: torch.Tensor, input_y: torch.Tensor):
        x = input_x
        y = input_y
        if self.ssm_branch:
            if self.post_norm:
                ...
                x, y = self.op(self.norm(x), self.norm(y))
                x_mid = input_x + self.drop_path(x)
                y_mid = input_y + self.drop_path(y)
        if self.mlp_branch:
            if self.post_norm:
                x = x + self.drop_path(self.norm2(self.mlp(x))) # FFN
            else:
                x = x_mid + self.drop_path(self.mlp(self.norm2(x_mid))) # FFN
                y = y_mid + self.drop_path(self.mlp(self.norm2(y_mid))) # FFN
        return x, y

    def _forward_none(self, input: torch.Tensor):
        x = input
        if self.ssm_branch:
            if self.post_norm:
                x = x + self.drop_path(self.norm(self.op(x)))
    ...
    def forward(self, input_x: torch.Tensor, input_y = None):
        if self.use_checkpoint:
            return checkpoint.checkpoint(self._forward, input_x, input_y)
        elif input_y is None:
            return self._forward_none(input_x)
        else:
...
class VSSM(nn.Module):
    def __init__(
        self, 
        input_shape=(32,32),
        patch_size=1,     #try to set 4(origin)
        in_chans_hsi=144,
        in_chans_lidar=1, 
        num_classes=1000, 
        depths=[9], 
        ...
        downsample_version: str = "v_my", # "v1", "v2", "v3" "v_my"
        ...
        **kwargs,
        ...
        self.patch_embed_hsi = _make_patch_embed(in_chans_hsi, dims, patch_size, patch_norm, norm_layer, channel_first=self.channel_first)
        self.patch_embed_lidar = _make_patch_embed(in_chans_lidar, dims, patch_size, patch_norm, norm_layer, channel_first=self.channel_first)

        # _make_downsample = dict(
        #     v1=PatchMerging2D, 
        #     v2=self._make_downsample, 
        #     v3=self._make_downsample_v3, 
        #     v_my=self._make_downsample_my,
        ...
        for i_layer in range(self.num_layers):
            ...
            self.layers_x.append(self._make_layer(
                dim = self.dims,
                drop_path = dpr[0:sum(depths[:i_layer + 1])],
        ...
        for i_layer in range(self.num_layers):
            ...
            self.layers_y.append(self._make_layer(
                dim = self.dims,
                drop_path = dpr[0:sum(depths[:i_layer + 1])],
                use_checkpoint=use_checkpoint,
                norm_layer=norm_layer,
        ...
        for i_layer in range(self.num_layers):
            ...
            self.layers_z.append(self._make_layer_add(
                dim = self.dims,
                drop_path = dpr[0:sum(depths[:i_layer + 1])],
                use_checkpoint=use_checkpoint,
                norm_layer=norm_layer,
        ...
        # self.classifier_hsi = nn.Sequential(OrderedDict(
        ...
        # ))
        # self.classifier_lidar = nn.Sequential(OrderedDict(
        #     norm=norm_layer(self.num_features), # B,H,W,C  2 2 2 768
        #     permute=(Permute(0, 3, 1, 2) if not self.channel_first else nn.Identity()),
        #     avgpool=nn.AdaptiveAvgPool2d(1),
        #     flatten=nn.Flatten(1),
        #     head=nn.Linear(self.num_features, num_classes),
        # ))
        # self.classifier_add = nn.Sequential(OrderedDict(
        #     norm=norm_layer(self.num_features), # B,H,W,C
        #     permute=(Permute(0, 3, 1, 2) if not self.channel_first else nn.Identity()),
        #     avgpool=nn.AdaptiveAvgPool2d(1),
        #     flatten=nn.Flatten(1),
        ...
        self.classifier_hsi = nn.Sequential(OrderedDict(
            norm=norm_layer(self.num_features), # B,H,W,C
            permute=(Permute(0, 3, 1, 2) if not self.channel_first else nn.Identity()),
            head=nn.Conv2d(self.num_features, num_classes, 1, 1) if num_classes > 0 else nn.Identity(),
    ...
    @staticmethod
    def _make_downsample_my(dim=96, out_dim=192, norm_layer=nn.LayerNorm, channel_first=False):
        # if channel first, then Norm and Output are both channel_first
        return nn.Sequential(
            (nn.Identity() if channel_first else Permute(0, 3, 1, 2)),
    ...
    def forward(self, x: torch.Tensor, y: torch.Tensor):
        ...
        y = self.patch_embed_lidar(y)
        z = x + y
        for (layer_x, layer_y, layer_z) in zip(self.layers_x, self.layers_y, self.layers_z):
            for i, (block_x, block_y, block_z) in enumerate(zip(layer_x, layer_y, layer_z)):
                x, _ = block_x(x, z)
                y, _ = block_y(y, z)
                z = block_z(x + y)               
        x = self.classifier_hsi(x)
        y = self.classifier_lidar(y)
        z = self.classifier_add(z)
        #######     feature visualization    ######## 
        # feature_map_x = x.clone().detach()
        # feature_map_y = y.clone().detach()
        # feature_map_z = z.clone().detach()
        # self.visualize_feature_map(feature_map_x, 'mm_vit_code/x_clip_average_feature_map.png')
        # self.visualize_feature_map(feature_map_y, 'mm_vit_code/y_clip_average_feature_map.png')
        # self.visualize_feature_map(feature_map_z, 'mm_vit_code/z_C_6_feature_map.png')
        #######     feature visualization    ########    
        return x, y, z
    
    def visualize_feature_map(self, feature_map, filename):
        # 假设 feature_map 的尺寸是 [B, C, H, W]，即 [批次大小, 通道数, 高度, 宽度]
        # 我们这里随机选择一个样本和几个通道进行可视化
        batch_idx = 0  # 选择第一个batch中的样本
        num_channels = min(feature_map.shape[1], 100)  # 确保最多显示6个通道

        fig, axes = plt.subplots(1, num_channels, figsize=(15, 15))
        
        for i in range(num_channels):
            ax = axes[i]
            # 通过矩阵对齐方式避免单个轴无法显示多通道问题
            if len(axes) > 1:
                ax = axes[i] 
            feature_map_single_channel = feature_map[batch_idx, i].cpu().numpy()
            ax.imshow(feature_map_single_channel, cmap='viridis')
            ax.axis('off')
            ax.set_title(f'Channel {i+1}')
        
        # feature_map_batch = feature_map[batch_idx].cpu().numpy()
        # composite_feature_map = feature_map_batch.mean(axis=0)
        # plt.figure(figsize=(12, 12))
        # plt.imshow(composite_feature_map, cmap='viridis')
        # plt.colorbar()
...
if __name__ == '__main__':
    device = 'cuda'
    x1 = torch.randn(size=(2, 144, 16, 16)).to(device)
    x2 = torch.randn(size=(2, 1, 16, 16)).to(device)

    net = VSSM(num_classes=16).to(device)
    y = net(x1, x2)
